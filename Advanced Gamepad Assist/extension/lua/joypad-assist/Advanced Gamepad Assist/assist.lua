...
local function calcCorrectedSteering(vData, targetFrontSlipDeg, initialSteering, absInitialSteering, assistFadeIn, dt)
    ...
    uiData._maxLimitReduction  = math.lerp(0, finalTargetSlip, lib.clamp01(uiData.maxDynamicLimitReduction / 10.0)) -- math.lerp(0.8, 1.2, lib.clamp01(vData.localHVelLen / getTopSpeedEstimate(vData)))
    local angleSubLimit        = uiData._maxLimitReduction -- How many degrees the steering limit is allowed to reduce when the car oversteers, in the process of trying to maintain the desired front slip angle -- + math.max(0.0, -inputSign * selfSteerForce * vData.steeringLockDeg)
    local clampedFAxleVelAngle = lib.clampEased(inputSign * fAxleHVelAngle, -vData.steeringLockDeg, angleSubLimit, angleSubLimit / (vData.steeringLockDeg + angleSubLimit)) -- Limiting how much the front velocity angle can affect the steering limit
