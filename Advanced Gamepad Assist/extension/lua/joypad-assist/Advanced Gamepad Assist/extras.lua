...
M.update = function(vData, uiData, absInitialSteering, dt)
    ...
    if (uiData.triggerFeedbackL > 0.001 or uiData.triggerFeedbackR > 0.001) and vData.localHVelLen > 0.5 then
        ...
        if xbox ~= nil then
            ...
            if actualBrakeNd > 0.5 and M.controllerBrake > 0.001 and (vData.vehicle.absMode < 1 or uiData.triggerFeedbackAlwaysOn) then
                lVibration = ((math.lerpInvSat(actualBrakeNd, 0.5, 1.0) ^ 4.0) * 0.95 + 0.05) * uiData.triggerFeedbackL
            end

            if actualThrottleNd > 0.5 and M.controllerThrottle > 0.001 and (vData.vehicle.tractionControlMode < 1 or uiData.triggerFeedbackAlwaysOn) then
